...
OUTPUT_BASE_FOLDER = "/mnt/data/flavio_plates"
PROCESSED_LOG = os.path.join(OUTPUT_BASE_FOLDER, 'imagens_processadas_plates.txt')
...
def detect_plates(sess, image, threshold=0.5):
    img_expanded = np.expand_dims(image, axis=0)
    boxes, scores = sess.run([plate_boxes_tensor, plate_scores_tensor], feed_dict={plate_input_tensor: img_expanded})
...
for root, _, files in os.walk(INPUT_BASE_FOLDER):
    ...
    for file in files:
        ...
        try:
            ...
            detections = detect_plates(plate_sess, image, PLATE_THRESHOLD)
